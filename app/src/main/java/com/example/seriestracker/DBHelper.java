...
public class DBHelper extends OrmLiteSqliteOpenHelper {
    ...
    public static final String DB_NAME = "series_tracker.db";
    ...
    public void onCreate(SQLiteDatabase database, ConnectionSource connectionSource) {
        try {
            TableUtils.createTable(connectionSource,searchService.Detail.class);
    ...
    public <T> List<T> getFilteredMovies(Class<T> clazz,boolean status) throws SQLException {
        Dao<T, ?> dao = getDao(clazz);
        QueryBuilder<T, ?> queryBuilder = dao.queryBuilder();
        Where<T, ?> where = queryBuilder.where();
        SelectArg selectArg = new SelectArg();
        where.eq("Status",selectArg);
        PreparedQuery<T> preparedQuery = queryBuilder.prepare();

        selectArg.setValue(status);
